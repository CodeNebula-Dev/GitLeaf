...
export class InviteManager {
  ...
  public async acceptInviteAsync(
    ...
    if (invite) {
      ...
      // 2. Clone via Git remote (the reliable path!)
      if (invite.gitRemote) {
        return this.cloneViaGit(invite, collaboratorName);
      }

      // 3. Fallback: Fetch via HTTP from host's running server
      return this.fetchViaHttp(invite, collaboratorName);
    }

    // 4. If the code isn't in our local registry, try HTTP to localhost
    //    (maybe the code was generated by the running server)
    try {
    ...
    throw new Error(
      `Invite code "${clean}" not found. Make sure the project owner's GitLeaf server is running, ` +
      `or that the project has a linked GitHub remote.`
    );
  }
